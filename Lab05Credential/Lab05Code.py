...
def credential_EncryptUserSecret(params, pub, priv):
    ...
    c = to_challenge([g, pub, a, b, w0*g, w0*pub + w1*g, w2*g])
...
def credential_Issuing(params, pub, ciphertext, issuer_params):
    ...
    #   (x0*u) + (v*x1b)*g 
    # valToEnc =  ((rand*x0)*g + (x1*rand*v) *g )
    
    # rng = o.random()
    
    # given in NIZK statement
    partialA = x1b * a
    partialB = x1b * b + x0 * u 
    # TODO 3
    
    r_prime = o.random()
    randEncA, randEncB = (r_prime*g, r_prime*pub)
    
    new_a = randEncA + partialA
    new_b = randEncB + partialB
...
def credential_show(params, issuer_pub_params, u, u_prime, v):
    ...
    c = to_challenge([g,h,u, X1, Cup, Cv, WV, WC])
...
def credential_show_verify(params, issuer_params, tag, proof):
    ...
    c_prime = to_challenge([g,h,u,X1,Cup,Cv,WV,WC])
...
def credential_show_pseudonym(params, issuer_pub_params, u, u_prime, v, service_name):
    ...
    # pseudonym is valid if response used for 
    ...
    c = to_challenge([g,h,u, X1, Cup, Cv, WV, WC, WPsuedo])
...
def credential_show_verify_pseudonym(params, issuer_params, pseudonym, tag, proof, service_name):
    ...
    WV = c*V + (rz1*X1) + (rr*g)    
    WC = c*Cv +  rv*u + rz1*h
    WPsuedo =  (rv*N) + (c*pseudonym)


    c_prime = to_challenge([g,h,u,X1,Cup,Cv,WV,WC,WPsuedo])
